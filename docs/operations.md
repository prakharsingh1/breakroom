...
Sandbox migration 5 adds five tables (21 total). The restore smoke now includes encrypted agent source/provider keys, jobs, trial evidence and worker records, verifies source decryption/hash integrity with an independently retained synthetic vault key, and checks project cascades. Production vault keys need a separate protected backup. Treat restored worker heartbeats as stale and do not replay interrupted model calls; review the [sandbox recovery and supervisor requirements](sandbox.md).
